...
class Connector:
    ...
    def __init__(self, application_parameters, connector_main_path, uploads_path, media_path):
    ...
    def renaming_uuid_in_photo_files(self, uuid_assignment, origin_uuid=None):
        ...
        # List of file names
        file_names = os.listdir(self.media_path)
        ...
        if uuid_assignment == "jpg_pairs":
            ...
            for file_num, file_names in file_num_dict.items():
                ...
                for file_name in file_names:
                    self.change_image_uuid(os.path.join(self.media_path, file_name), originUUID)
    ...
    def photos_drag_and_drop_and_upload(self, attraction_names):
        ...
        try:
            ...
            for number in file_dict.keys():
                ...
                print("new file names numbers", "new file names numbers: " + str(new_numbers))

            print("All photos renamed",
                      "----------------------------------------------------- All photos renamed and copy to upload folder --------------------------------------------------------------")

        except (FileNotFoundError, PermissionError) as e:
            print("Source directory error",
                      f"An error occurred while accessing the source directory {source_dir}: {e}")
            ...
            for Connector.file_name in file_names:
                ...
                if Connector.file_name.endswith(".mp4"):
                    try:
                        ...
                        print("File processing error",
                                  f"An error occurred while processing file {Connector.file_name}: {e}")
    ...
    def change_video_file_name_media_testing(self, file_name, new_number):
        ...
        os.rename(self.media_path + "\\" + file_name,
                  self.media_path + "\\" + new_file_name)

        return self.media_path + "\\" + new_file_name
    ...
    def find_media(self):
        # Make a list of files for the relevant source_path
        file_names = os.listdir(self.media_path)
        print(file_names)
        return file_names

    def change_date_photo_file_name(self, file_names):
        # Rename photo date file for this structure '2023-02-16-MT-EW-00001'
        for file_name in file_names:
            parts = file_name.split("-")
            date = parts[0] + "-" + parts[1] + "-" + parts[2]
            ...
            print("file re-named to : " + str(new_file_name))

            os.rename(self.media_path + "\\" + file_name,
                      self.media_path + "\\" + new_file_name)

        return [self.media_path + "\\" + new_file_name for new_file_name in file_names]

    def drag_and_drop_media_to_uploads_folder(self):
        pass
